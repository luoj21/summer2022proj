...
```{r}
#setwd("/Users/luoj8/Documents/project_2022/braincptac3")
library(tidyverse)
library(ggpubr)
library(skimr)
library(EnvStats)
...
``` {r}
 ggplot(test, aes(value, color = type)) + 
   geom_density() + 
...
### Linear Regression for Protein and mRNA
``` {r}
 test2 <- manifest_RNA %>%
 ...
 ggplot(subset(test3, gene_name %in% c("RANBP1", "RANGRF")), aes(fpkm_unstranded, value, color = gene_name)) +
...
### Survival Curve Between Male and Female
``` {r}
...
brain_survfit2 <- protein_shared %>%
   ...
   filter(Gene == "RANGRF") %>% ### Can change to RANBP1
...
# Can change to see low/high of male and female
brain_survfit3 <- brain_survfit2 %>%
...
ggsurvplot(kmfit3, data = brain_survfit3, conf.int = TRUE, risk.table = TRUE, pval = TRUE) +
   xlab("Time in Months") +
   ggtitle("RANGRF Gene Survival Curve (Female)")
```


### Survival Curve Separated by High/Low Male and Female Expression
``` {r}
kmfit2 <- survfit(Surv(time_recent_visit, vital_status) ~ value_status, data = brain_survfit2, type = "kaplan-meier")
...
ggsurvplot(kmfit2, 
            ...
            legend.title= "") + 
   xlab("Time in Months") +
   ggtitle("RANGRF1 Gene Brain Cancer Survival Curve")
